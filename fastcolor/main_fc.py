...
cutoff_time=60
print("Cutoff time is:", cutoff_time)
...
result_file= open('results_fastcolor.txt', 'w')
graph_count=0
benchmarks=glob.glob('benchmarks/*.col')
num_graphs=len(benchmarks)
num_samples=50 #number of samples to run on each graph
#testing on all the graphs, note that the cutoff time is 60 seconds and the number of samples is 50, it may take time. change these values for faster results
for file_name in benchmarks:
    ...
    result_file.write(f'{file_name.split("/")[-1]}: {best_solution} {best_time} {mean_sol} {stdev_sol} {mean_time} {stdev_time}\n')
    result_file.flush()
