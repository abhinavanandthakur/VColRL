...
graphs,_=read_data(filepath,device)

#split dataset into train, validation and test

train_graphs,test_graphs,val_graphs=split_list(graphs)
print(len(graphs))
#optimum_soln=optimum_soln[4500:]
#utility for saving images
save_freq=140#len(train_graphs)//rollout_batch_size
...
#utility files for logging validation stats
...
for update_t in range(max_update_t):
    ...
    if (update_t + 1) % log_freq == 0:
        ...
        if (update_t + 1) % vali_freq == 0:
            ...
            vali_file.write('{:.4f}'.format(sol.item())+'\n')
            print("vali stats...")
