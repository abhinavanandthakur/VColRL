...
class ProxPolicyOptimFramework(object):
    def __init__(
        ...
        #self.epoch=0
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=10)  # T_max is the number of epochs
        #self.optimizer = Adam(actor_critic.parameters(), lr=init_lr)
        #self.optimizer = SGD(actor_critic.parameters(), lr=0.01, momentum=0.9)
        #self.optimizer = LBFGS(actor_critic.parameters(), lr=1)
    ...
    def update(self, rollout):
        ...
        avg_entropy_loss= torch.tensor(0.0)
        avg_objective=torch.tensor(0.0)
        cnt = 0
        ...
        for samples in data_loader:
            ...
            node_masks = (obs.select(2, 0).long() == self.num_channels+1)
            ...
            # compute critic loss
            critic_loss =  (value_preds - rets).pow(2).mean()
            ...
            self.optimizer.step()
            #print(self.optimizer.param_groups[-1]['lr'])
            ...
            avg_entropy_loss += entropy.to("cpu")
            avg_objective+= loss.to("cpu")
            cnt += 1
            torch.cuda.empty_cache()

        avg_actor_loss /= cnt 
        avg_critic_loss /= cnt 
        avg_entropy_loss /= cnt
        avg_objective/=cnt
        
        return avg_actor_loss, avg_critic_loss, avg_entropy_loss,avg_objective
